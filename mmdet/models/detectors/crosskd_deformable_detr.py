...
class CrossKDDeformableDETR(DeformableDETR):
    ...
    def forward_transformer_with_kd(self,
        ...
        stu_query_list = [decoder_inputs_dict.pop('query')]
        stu_query_list.extend(
            decoder_outputs_dict['hidden_states'].unbind(dim=0))
        decoder_inputs_dict['query_list'] = stu_query_list
        decoder_inputs_dict.pop('reference_points')
        decoder_inputs_dict['reference_points_list'] = \
            decoder_outputs_dict['references']
        decoder_inputs_dict['stu_memory'] = decoder_inputs_dict.pop('memory')
        decoder_inputs_dict['tea_memory'] = tea_enc_outputs_dict['memory']
        kd_inputs_dict = self.cross_forward_teacher_decoder(**decoder_inputs_dict)
        
        return head_inputs_dict, kd_inputs_dict
    
    def cross_forward_teacher_decoder(
            self, query_list, query_pos, stu_memory, tea_memory,
            memory_mask, reference_points_list, spatial_shapes,
        ...
        for layer_id, layer in enumerate(decoder.layers):
            query = query_list[layer_id]
            reference_points = reference_points_list[layer_id]
            ...
            tea_intermediate.append(tea_query)
        ...
        kd_inputs_dict['references'] = reference_points_list
